pipeline {
	 ...
	 stages {
		...
		stage('Sonar Analysis'){
			steps{
				bat "\"${tool 'sonar-msbuild'}\"\\SonarScanner.MSBuild.exe begin /k:testSonarqube /d:sonar.verbose=true /d:sonar.host.url=http://192.168.1.1253:9000 /d:sonar.login=admin /d:sonar.password=123"
